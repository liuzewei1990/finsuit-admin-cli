...
export default {
  ...
  data () {
    return {
      tableApi: this.$api.getUserManages,
      query: {
        page: 1,
        limit: 20,
        status: "TRUE"
  ...
  methods: {
    ...
    async handleDeleteAction (rowData) {
      this.$confirm("此操作将删除该条管理员数据, 是否继续?", "提示", { type: "warning" }).then(async () => {
        let data = await this.$api.deleteUserManage(rowData.username);
        if (data.code == "00") {
    ...
    async handleClickAddSaveBtn (formName) {
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          ...
          let data = await this.$api.postAddUser({ ...this.addForm });
          if (data.code == "00") {
    ...
    async handleClickEditSaveBtn (formName) {
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          this.saveLoading = true;
          let data = await this.$api.patchEditUser({ ...this.editForm });
          if (data.code == "00") {
    ...
    async handleRoleAction (rowData) {
      ...
      let data = await this.$api.getRolesTree({ username: rowData.username });
    ...
    async handleClickRoleSaveBtn () {
      ...
      let data = await this.$api.patchConfigRole({
